...
# TODO: Typespecs for node args everywhere below
...
class GenBase:
    ...
    def __init__(self, sender, ref, node):
        self.node_ = node
        ...
        self.ref_ = ref
        """ An unique ref generated by the caller. A ``term.Reference`` object. 
        """

    async def reply(self, local_pid, result):
        """ Reply with a gen:call result
        """
        self.node_.send(sender=local_pid,
                        receiver=self.sender_,
                        message=(self.ref_, result))

    async def reply_exit(self, local_pid, reason):
        ...
        await self.node_.dist_command(
            receiver_node=self.sender_.node_.text_,
            message=reply)
...
class GenIncomingMessage(GenBase):
    ...
    def __init__(self, sender, ref, message, node):
        GenBase.__init__(self, sender=sender, ref=ref, node=node)
...
class GenIncomingCall(GenBase):
    ...
    def __init__(self, mod, fun, args, group_leader, sender, ref, node):
        GenBase.__init__(self, sender=sender, ref=ref, node=node)
        self.mod_ = mod
        """ Module name as atom. """
        self.fun_ = fun
...
def parse_gen_message(msg, node):
    ...
    return GenIncomingMessage(sender=msender, ref=mref, message=gcmsg,
                              node=node)
