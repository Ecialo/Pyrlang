...
""" Raw Python implementation for ETF encoding/decoding routines
"""
...
ETF_VERSION_TAG = 131  # also defined in etf.py
...
def incomplete_data(where=""):
    ...
    if where:
        ...
        raise RuntimeError("Incomplete data at " + where)
...
def binary_to_term_2(data: bytes, options: dict = None):
    """ Proceed decoding after leading tag has been checked and removed.
        ...
        :raises RuntimeError: on various errors or on an unsupported tag
    """
    if options is None:
        options = {}
    ...
    if tag == TAG_LIST_EXT:
        ...
        result_tail, tail = binary_to_term_2(tail)

        if options.get("simple_lists", True):
            return result, tail

        return term.List(elements=result, tail=result_tail), tail
    ...
    raise RuntimeError("Unknown tag %d" % data[0])
...
__all__ = ['binary_to_term_2', 'term_to_binary_2']
